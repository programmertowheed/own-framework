<!--Header-->
<?php includes("backend.partials.header", ["page_title" => "Edit role"]) ?>
                                    ...
                                    if ($permission['group_name'] == $group['group_name']) {
                                        ...
                                        <div>
                                            <input type="checkbox" class="cursor-pointer"
                                                   ...
                                                   onclick="checkSinglePermission('role-<?= $i ?>-management-checkbox','<?= $group['id'] ?>Management','<?= $group['groupTotal'] ?>','<?= $permission['id'] ?>', this)">
                                            <label class="cursor-pointer"
...
<script type="text/javascript">
    ...
    let role_id = <?= json_encode($role['id']) ?>;
    let group_permissions = <?= json_encode($permission_groups) ?>;
    let allPermissions = <?= json_encode($permissions) ?>;
    let havePermissions = <?= $role_permissions ?>;
    ...
    $(document).ready(function () {
        ...
        havePermissions.forEach((el) => {
            $('#checkPermission' + el).prop('checked', true);
        });

        checkPermissionChecked();
    });

    //checked selected permission
    const checkPermissionChecked = () => {
        group_permissions.forEach((group) => {
            let newArr = [];
            allPermissions.map((p) => {
                if (p.group_name == group.group_name) {
                    newArr.push(p.id);
                }
            });
            let select = true;
            newArr.map(p => {
                if (!havePermissions.includes(p)) {
                    select = false;
                }
            });

            if (select) {
                const groupID = "#" + group.id + "Management";
    ...
    $('#handleSubmit').click(function (e) {
        ...
        if (name == "") {
            ...
            axios.post("<?= API_URL ?>/update/role-permission", formData).then(function (response) {
